...
# 2. ÜRETİM PARAMETRELERİ (Orijinal Tabloya Tam Sadık Kalındı)
DK_UCRETI = 25.0       
PIERCING_SURESI = 2.0  
KG_UCRETI = 45.0       
...
with st.sidebar:
    ...
    # Hız ataması (Tablodaki değer yoksa en yakın alt değeri alır)
...
if uploaded_file:
    ...
    # Kontur ve Hiyerarşi (RETR_CCOMP: İç/Dış ayrımı için idealdir)
    ...
    if contours and hierarchy is not None:
        ...
        for i, cnt in enumerate(contours):
            # Sadece ana objeleri ve ilk seviye delikleri say (Çizgi kalınlığını piercing saymaz)
            if hierarchy[0][i][3] == -1 or hierarchy[0][i][3] == 0:
                cevre = cv2.arcLength(cnt, True)
                if cevre * oran > 5.0: # 5mm altı gürültüleri ele
