...
# ==========================================
# 1. YARDIMCI FONKSİYONLAR (PDF & HESAPLAMA)
# ==========================================
...
def hesapla_ve_goster(kesim_m, kontur_ad, alan_mm2, w_real, h_real, result_img_bgr, metal, kalinlik, adet, guncel_hiz, plaka_adi):
    """
    Tüm sayfalarda ortak kullanılan hesaplama ve gösterim motoru.
    """
    # Global parametreler
    ...
    # Session'dan güncel birim fiyatı al
...
# ==========================================
# 2. AYARLAR VE KÜTÜPHANE KONTROLLERİ
# ==========================================
try:
    import ezdxf
...
with st.container():
    ...
    with c_h2:
        kalinlik = st.selectbox("2. Kalınlık (mm)", materials.VERİ[metal]["kalinliklar"])
...
# --- FOTOĞRAF ANALİZ SAYFASI ---
    ...
    # Mobilde ayarları gizlemek için expander
    ...
    if 'gecici_gorsel' in st.session_state and st.session_state.gecici_gorsel:
        ...
        if original_img is not None:
            ...
            if valid_contour_list:
                ...
                st.warning("Kesim yolu bulunamadı. Hassasiyeti değiştirin.")
        else:
...
# --- DXF ANALİZ SAYFASI ---
    ...
    if not dxf_active:
        st.error("Gerekli kütüphaneler (ezdxf) eksik.")
        ...
        try:
            ...
            if w_real > 0:
                ...
                if valid_cnts:
                    ...
                    _, _, w_p, _ = cv2.boundingRect(all_pts)
                    ...
                    alan_mm2 = w_real * h_real # Yaklaşık alan
                    
                    hesapla_ve_goster(kesim_m, piercing_basi, alan_mm2, w_real, h_real, result_img,
                                      metal, kalinlik, adet, guncel_hiz, secilen_plaka_adi)
            else:
                st.warning("DXF içeriği boş veya okunamadı.")
        except Exception as e:
            st.error(f"Hata: {e}")
